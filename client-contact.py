...
def navigate_to_MAXIS_screen(function_to_go_to, command_to_go_to):
    transmit()
    MAXIS_check = bzio.ReadScreen(5, 1, 39)
    ...
    if MAXIS_check == "MAXIS" or MAXIS_check == "AXIS ":
        ...
        if row != 0:
            ...
            if row != 0:
                ...
                current_case_number = ""
        print(MAXIS_function)
        if current_case_number == MAXIS_case_number and MAXIS_function == function_to_go_to and STAT_note_check != "NOTE":
            ...
            print(function_to_go_to)
            bzio.WriteScreen("________", 18, 43)
            bzio.WriteScreen(MAXIS_case_number, 18, 43)
            bzio.WriteScreen(MAXIS_footer_month, 20, 43)
            bzio.WriteScreen(MAXIS_footer_year, 20, 46)
            bzio.WriteScreen(command_to_go_to, 21, 70)
            print(command_to_go_to)
            transmit()
...
class MAXISClientContact (wx.Frame):
    ...
    def __init__(self, parent):
        wx.Frame.__init__(self, parent, id=wx.ID_OPEN, title=u"Client Contact", pos=wx.DefaultPosition, size=wx.Size(-1, -1),
        ...
        self.combo_contact_type = wx.ComboBox(self.m_panel2, wx.ID_OPEN, wx.EmptyString, wx.DefaultPosition, wx.DefaultSize, combo_contact_typeChoices, 0)
        ...
        combo_phone_numberChoices = phone_list
        ...
        sbSizer1 = wx.StaticBoxSizer(wx.StaticBox(self.m_panel2, wx.ID_ANY, u"Helpful info for call centers (or front desks) to pass on to clients"),
                                     wx.VERTICAL)
        ...
        self.checkbox_tikl = wx.CheckBox(self.m_panel2, wx.ID_ANY, u"Check here if you want to TIKL out for this case after the case note is done.",
                                         wx.DefaultPosition, wx.DefaultSize, 0)
        gbSizer1.Add(self.checkbox_tikl, wx.GBPosition(10, 0), wx.GBSpan(1, 4), wx.ALL, 5)

        self.checkbox_CAF1_reminder = wx.CheckBox(self.m_panel2, wx.ID_ANY, u"Check here if you reminded client about the importance of the CAF 1. ",
                                                  wx.DefaultPosition, wx.DefaultSize, 0)
        ...
        self.checkbox_callback = wx.CheckBox(self.m_panel2, wx.ID_ANY, u"Check here if you left a voicemail requesting a call back.",
                                             wx.DefaultPosition, wx.DefaultSize, 0)
        ...
        self.checkbox_callctr_answered = wx.CheckBox(sbSizer2.GetStaticBox(), wx.ID_ANY, u"Answered caller's question.",
                                                     wx.DefaultPosition, wx.DefaultSize, 0)
        sbSizer2.Add(self.checkbox_callctr_answered, 0, wx.ALL, 5)

        self.checkbox_callctr_transfer = wx.CheckBox(sbSizer2.GetStaticBox(), wx.ID_ANY, u"Transferred caller to Worker.",
                                                     wx.DefaultPosition, wx.DefaultSize, 0)
    ...
    def click_OK(self, event):
        ...
        global tikl_checkbox
        tikl_checkbox = self.checkbox_tikl.GetValue()
...
class PRISMClientContact (wx.Frame):
    ...
    def __init__(self, parent):
        wx.Frame.__init__(self, parent, id=wx.ID_ANY, title=u"Client Contact", pos=wx.DefaultPosition, size=wx.Size(-1, -1),
        ...
        sbSizer3 = wx.StaticBoxSizer(wx.StaticBox(self.m_panel3, wx.ID_ANY, u"Select on contact type from this group, based on CAAD note requirement"),
                                     wx.VERTICAL)
        ...
        choice_cp_contactChoices = [u"T0050 PHONE CALL TO CP",
                                    u"T0051 PHONE CALL FR CP",
                                    u"T0052 PHONE CALL RET TO CP",
                                    u"T0053 PHONE CALL RET FR CP",
                                    u"T0054 PHONE CALL ATMPT TO RET TO CP",
                                    u"T0093 CONTACT WITH CP SPOUSE",
                                    u"T0101 PHONE CONTACT CP'S ATTORNEY",
                                    u"T0201 CONTACT WITH CP EMPLOYER",
                                    u"M3910 INTERVIEW WITH CP",
        ...
        choice_ncp_contactChoices = [u"T0058 PHONE CALL RET FR NCP",
                                     u"M2122 LETTER RECD FROM NCP",
                                     u"M3911 INTERVIEW WITH NCP",
                                     u"T0092 CONTACT WITH NCP SPOUSE",
                                     u"T0069 PHONE CALL ATMPT RET TO NCP AY",
                                     u"T0068 PHONE CALL RET FR NCP AY",
                                     u"T0067 PHONE CALL RET TO NCP AY",
                                     u"T0066 PHONE CALL FR NCP AY",
                                     ...
                                     u"T0061 PHONE CALL FROM NCP EMP",
                                     u"T0060 PHONE CALL TO NCP EMP",
                                     u"T0059 PHONE CALL ATMT TO RET TO NCP",
                                     u"T0058 PHONE CALL STMT TO RET TO NCP",
                                     u"T0057 PHONE CALL RET TO NCP",
                                     u"T0056 PHONE CALL FR NCP",
        ...
        choice_other_contactChoices = [u"T0111 CONTACT WITH OTHER STATE AGENCY",
                                       u"T0105 PHONE CONTACT WITH CSPC",
                                       u"T1107 CONTACT WITH VITAL RECORDS",
                                       ...
                                       u"T0074 CONTACT WITH STATE HELP DESK",
                                       u"T0070 PHONE CALL/OTHER",
        ...
        self.date_of_contact = wx.adv.DatePickerCtrl(self.m_panel3, wx.ID_ANY, wx.DefaultDateTime, wx.DefaultPosition, wx.DefaultSize, wx.adv.DP_DROPDOWN)
        ...
        self.text_SpecialInstructions = wx.StaticText(sbSizer5.GetStaticBox(), wx.ID_ANY, u"Special instructions for client:",
                                                      wx.DefaultPosition, wx.DefaultSize, 0)
        ...
        self.checkbox_msg_callback = wx.CheckBox(self.m_panel3, wx.ID_ANY, u"Check here if if you left a generic message requesting they return call.",
                                                 wx.DefaultPosition, wx.DefaultSize, 0)
...
# This is not currently used - may use this in the future to reduce lines of code
...
worker_type = "MAXIS"		# Manually defined at this time until functionality created to programatically determine this
phone_list = ["651-234-4567", "651-555-7890", "651-555-1234"]

# Find Case number
global MAXIS_case_number
MAXIS_case_number = MAXIS_case_number_finder()
MAXIS_case_number = str(MAXIS_case_number)
footer_mo_yr = MAXIS_footer_finder()
MAXIS_footer_month = footer_mo_yr[0]
MAXIS_footer_year = footer_mo_yr[1]

print("Case Number: %s \n Footer Month : %s \n Footer Year: %s" % (MAXIS_case_number, MAXIS_footer_month, MAXIS_footer_year))
...
bzio.MsgBox("Client contact was run for a %s worker." % (worker_type))

if worker_type is "MAXIS":
    bzio.MsgBox("Case number: %s" % (MAXIS_case_number))

    start_a_blank_case_note()
    if contact_re is wx.EmptyString:
        write_variable_in_CASE_NOTE("%s %s %s" % (contact_type, contact_dir, contact_person))
    else:
        write_variable_in_CASE_NOTE("%s %s %s Re: %s" % (contact_type, contact_dir, contact_person, contact_re))
    if interpreter_checkbox:
        write_variable_in_CASE_NOTE("* Contact was made: %s w/ interperter" % (contact_time))
    else:
        write_variable_in_CASE_NOTE("* Contact was made: %s" % (contact_time))

    write_bullet_and_variable_in_CASE_NOTE("Phone Number", phone_number)
    write_bullet_and_variable_in_CASE_NOTE("MNSURE/IC number", mets_ic_number)
    write_bullet_and_variable_in_CASE_NOTE("Reason for contact", contact_reason)
    write_bullet_and_variable_in_CASE_NOTE("Actions taken", actions_taken)
    write_bullet_and_variable_in_CASE_NOTE("Verifs needed", verifs_needed)
    write_bullet_and_variable_in_CASE_NOTE("Instructions/Message for client", client_instructions)
    write_bullet_and_variable_in_CASE_NOTE("Case status", case_status)

    if caf_1_checkbox:
        write_variable_in_CASE_NOTE("* Reminded client about the importance of submitting the CAF 1.")
    if sent_arep_checkbox:
        write_variable_in_CASE_NOTE("* Sent forms(s) to AREP.")
    if call_center_answer_checkbox:
        write_variable_in_CASE_NOTE("* Call center answered caller's question.")
    if call_center_transfer_checkbox:
        write_variable_in_CASE_NOTE("* Call center transferred call to a worker.")
    if follow_up_checkbox:
        write_variable_in_CASE_NOTE("* Follow-up is needed.")

    write_variable_in_CASE_NOTE("---")
    write_variable_in_CASE_NOTE(worker_singature)

    if tikl_checkbox:
        bzio.MsgBox("The script will now naviagte to a TIKL.")
